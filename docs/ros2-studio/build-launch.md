...
**Debug…** opens Zed's native DAP configuration picker for breakpoints, stepping, and variable inspection. ROS Studio Build and Run continue to display compiler errors directly in the ROS Build and Run Output panel; no separate Problems panel is opened. A DAP configuration must be able to reach the program and its source files in the selected host or container environment.
